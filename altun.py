...
models_dir = f"tu1_models/{int(time.time())}/"
logdir = f"tu1_logs/{int(time.time())}/"
...
model = RecurrentPPO("MlpLstmPolicy", env, tensorboard_log=logdir, **p_params)
...
for _ in range(total_iterations):
    iters += 1
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name="RecurrentPPO_0.0003_pi=64,64_vf=64,64_lstm=1")
